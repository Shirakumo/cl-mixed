...
(defun list-segments ()
  (cffi:with-foreign-object (count 'size_t)
    (with-error-on-failure ()
      (mixed:list-segments count (cffi:null-pointer)))
    (cffi:with-foreign-object (args :pointer (cffi:mem-ref count 'size_t))
      (with-error-on-failure ()
        (mixed:list-segments count args))
      (sort (loop for i from 0 below (cffi:mem-ref count 'size_t)
