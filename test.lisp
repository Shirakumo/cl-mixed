  ...
  (with-edge-setup (file out samplerate :pathname mp3 :samples samples)
    (let* ((source (cl-mixed:make-source (cl-mpg123:buffer file)
      ...
      (cl-mixed:with-buffers samples (li ri lo ro)
        (setf (cl-mixed:output :left source) li)
        (setf (cl-mixed:output :right source) ri)
        (setf (cl-mixed:input 0 space) li)
        (setf (cl-mixed:output :left space) lo)
        (setf (cl-mixed:output :right space) ro)
        (setf (cl-mixed:input :left drain) lo)
        (setf (cl-mixed:input :right drain) ro)
