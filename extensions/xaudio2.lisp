...
(cffi:defcstruct (callback :conc-name callback-)
  ...
  (buffer-1 (:struct xaudio2:buffer))
  (buffer-2 (:struct xaudio2:buffer))
  (buffer-data-1 :char :count 4096)
  (buffer-data-2 :char :count 4096))
...
(defmethod initialize-instance :after ((drain drain) &key)
  ...
  (let* ((pack (mixed:pack drain))
    ...
    (setf (xaudio2:buffer-audio-data (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-1))
          (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-data-1))
    (setf (xaudio2:buffer-audio-data (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-2))
          (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-data-2))
...
(defmethod mixed:mix ((drain drain))
  (let ((pack (mixed:pack drain))
    ...
    (mixed:with-buffer-tx (data start size pack)
      (flet ((try (buffer i)
               (when (= 0 (xaudio2:buffer-size buffer))
                 (let ((size (min size 4096)))
                   ...
                   (setf (xaudio2:buffer-context buffer) i)
                   (com:check-hresult (xaudio2:voice-submit-source-buffer (source drain) buffer (cffi:null-pointer)))
                   (mixed:finish size)))))
        (when (< 0 size)
          (or (try (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-1) 1)
              (try (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-2) 2)))))))
...
(cffi:defcallback buffer-end :void ((callback :pointer) (context :uint64))
  (declare (ignore context))
  (let ((buffer (ecase (ldb (byte 32 32) context)
                  (1 (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-1))
                  (2 (cffi:foreign-slot-pointer callback '(:struct callback) 'buffer-2)))))
    (setf (xaudio2:buffer-size buffer) 0)))
