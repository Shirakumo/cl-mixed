  ...
  (unless (device drain)
    (let ((pack (mixed:pack drain)))
      (cffi:with-foreign-objects ((device :pointer)
                                  (format '(:struct org.shirakumo.fraf.mixed.wasapi.cffi:waveformat-extensible)))
        (loop for (samplerate channels encoding) in (queries pack)
              do (org.shirakumo.fraf.mixed.wasapi.cffi:encode-wave-format format samplerate channels encoding)
