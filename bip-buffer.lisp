...
(defmacro with-buffer-tx ((data start size buffer &key (direction :input) (size #xFFFFFFFF)) &body body)
  (let ((bufferg (gensym "BUFFER"))
    ...
    `(let* ((,bufferg ,buffer)
       ...
       ,(ecase direction
          ((:input :read)
           `(multiple-value-bind (,start ,size) (request-read ,bufferg ,size)
              ...
              (flet ((finish (,sizeg) (finish-read ,bufferg ,sizeg))
                ...
                ,@body)))
          ((:output :write)
           `(multiple-value-bind (,start ,size) (request-write ,bufferg ,size)
              (declare (ignorable ,start ,size))
